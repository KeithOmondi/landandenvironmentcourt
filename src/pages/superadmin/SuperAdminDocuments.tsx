// src/features/documents/SuperAdminDocuments.tsx
//
// Super-admin documents workspace.
//
// Scope is deliberately narrower than AdminDocuments:
//   - Pending review queue (fetchPendingDocuments)
//   - Approve / reject with a review note
//     (approveDocument / rejectDocument)
//
// It does NOT create, edit, submit, or delete — those are
// AdminDocuments concerns. Documents have no featured flag, so there is
// no feature toggle here. All HTTP lives in the slice.
...
/**
 * Format a date-only string ("YYYY-MM-DD") without going through
 * `new Date()`. See AdminDocuments.tsx for the full rationale — the
 * short version is that `new Date('2014-11-20')` parses as UTC
 * midnight, and re-rendering that in a local timezone can shift the
 * day by one.
 */
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-lg bg-white p-4 shadow-lg">
        <h3 className="mb-1 text-base font-semibold">Reject document</h3>
...
interface PendingRowProps {
  ...
  busy: boolean;
  onApprove: (item: DocumentSummary) => void;
  onReject: (item: DocumentSummary) => void;
}
...
const PublishedRow = ({ item }: { item: DocumentSummary }) => (
  <tr className="border-b border-gray-100">
    <td className="px-3 py-2">
      <div className="font-medium">{item.title}</div>
      <div className="text-xs text-gray-500 line-clamp-1">
...
const SuperAdminDocuments = () => {
  ...
  const [rejecting, setRejecting] = useState<DocumentSummary | null>(null);
  ...
  useEffect(() => {
    if (tab === 'published') dispatch(fetchAllDocuments());
  }, [tab, dispatch]);
  ...
  const handleApprove = (item: DocumentSummary) => {
    dispatch(approveDocument({ documentId: item.id }));
  };
  ...
  const handleRejectConfirm = async (note: string) => {
    ...
    try {
      await dispatch(
        rejectDocument({ documentId: rejecting.id, reviewNote: note }),
      ).unwrap();
      setRejecting(null);
    } catch {
  ...
  return (
    <div className="mx-auto max-w-6xl p-4">
      <div className="mb-4">
        <h1 className="text-xl font-semibold">Documents review</h1>
