// src/features/publications/AdminPublications.tsx
//
// Admin publications workspace.
//
//   - Lists every publication, any status (fetchAllPublications).
//   - Opens one publication in an editor (fetchPublicationById /
//     createPublication / updatePublication).
//   - Submits drafts/rejected for review (submitPublication).
//   - Deletes the caller's own drafts/rejected (deletePublication).
...
const statusBadgeClass: Record<PublicationStatus, string> = {
...
const emptyInput: PublicationInput = {
  title: '',
  description: '',
...
// Derive the initial form value from the publication prop. Used as the
// lazy initializer for useState so we never call setState in an effect.
const formFromPublication = (
  ...
  publication
    ? {
        ...
        filePublicId: publication.filePublicId,
        fileBytes: publication.fileBytes,
...
// ─── Editor ──────────────────────────────────────────────────────────────────
//
// Rendered with `key={publication?.id ?? 'new'}` from the parent, so
// switching publications remounts this component and resets form state.
...
const PublicationEditor = ({ publication, onClose }: EditorProps) => {
  ...
  const [form, setForm] = useState<PublicationInput>(() =>
    formFromPublication(publication),
  );
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const result = await dispatch(uploadPublicationFile(file)).unwrap();
    setForm((prev) => ({
      ...prev,
  ...
  const handleSave = async () => {
    try {
      if (publication) {
        await dispatch(
          updatePublication({ publicationId: publication.id, payload: form }),
        ).unwrap();
      } else {
        await dispatch(createPublication(form)).unwrap();
      }
    } catch {
  ...
  // A publication can't be saved without a file — the validator
  // requires a non-empty fileUrl. Show the button disabled rather than
  // letting the user submit and get a 400.
  const canSave = form.fileUrl.trim().length > 0 && form.filePublicId.trim().length > 0;

  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-lg font-semibold">
          {publication ? `Edit: ${publication.title}` : 'New publication'}
        </h2>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="grid gap-3">
        ...
        <div className="flex gap-2">
          ...
          >
            {isSaving ? 'Saving…' : publication ? 'Save changes' : 'Create draft'}
          </button>
          {!canSave && (
...
const PublicationRow = ({ item, isSuperAdmin, onEdit }: RowProps) => {
  const dispatch = useDispatch<AppDispatch>();
  const { isDeleting, isReviewing } = useSelector(
    (s: RootState) => s.publications,
  );
  ...
  const handleSubmit = () => dispatch(submitPublication(item.id));

  const handleApprove = () => {
    const note = prompt('Review note (optional):') ?? undefined;
    dispatch(
      approvePublication({ publicationId: item.id, reviewNote: note || undefined }),
    );
  };
  ...
  const handleReject = () => {
    ...
    if (!note) return;
    dispatch(rejectPublication({ publicationId: item.id, reviewNote: note }));
  };
...
const AdminPublications = () => {
  ...
  // When the user clicks Edit, load the full publication into `current`.
  useEffect(() => {
    if (editingId) dispatch(fetchPublicationById(editingId));
  }, [editingId, dispatch]);

  const showEditor = isCreating || editingId !== null;
  const editorPublication = editingId ? current : null;
  ...
  return (
    <div className="mx-auto max-w-6xl p-4">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-xl font-semibold">Publications admin</h1>
        <button
          type="button"
          onClick={openCreate}
